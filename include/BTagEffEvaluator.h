...
class BTagEffEvaluator {
    public:
        ...
        BTagEffEvaluator(std::string path, std::vector<double> thresholds) {
            ...
            for (auto flav: flavMap) {
                ...
                TH2D* hist = dynamic_cast<TH2D*>(file->Get(("1lep_4j_jet_pt_eta_" + flav.second + "_wpM_eff").c_str()));
                hist->SetDirectory(0);
        ...
        float evaluate(int flavour, float bTag, float pT, float eta, float SFM) const {
            ...
            float effM = hist->GetBinContent(hist->FindBin(pT, eta));
            if (bTag <= _thresholds.at(0)) {
